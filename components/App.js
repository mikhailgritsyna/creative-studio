  ...
  return (
    <div className="relative overflow-hidden" style={{ background: "#1a1814", border: "1px solid #2d2a25", borderRadius: "10px" }}>
      ...
      <div className="flex items-center justify-between px-5 py-4" style={{ borderBottom: "1px solid #2d2a25" }}>
        ...
        <button onClick={handleCopy} className="flex items-center gap-1.5 text-xs px-3 py-1.5 rounded transition-colors"
      ...
      <div className="px-5 py-5">
        <p className="text-sm leading-relaxed whitespace-pre-wrap" style={{ color: "#c4b99a", fontFamily: "'Outfit', sans-serif" }}>{content}</p>
      </div>
    </div>
  );
}
        ...
        <div className="flex-1 overflow-y-auto">
          {history.length === 0 ? (
            <div className="flex items-center justify-center h-32">
              <p className="text-sm" style={{ color: "#5a5248" }}>Пока нет сохранений</p>
            </div>
          ) : history.map((item) => (
...
const Spinner = () => (
  <div className="flex flex-col items-center justify-center py-20 gap-6">
    ...
    <div className="text-center">
      <p className="text-sm font-semibold" style={{ color: "#8B7355" }}>Генерирую креатив</p>
      <p className="text-xs mt-1" style={{ color: "#5a5248" }}>Claude анализирует данные...</p>
    </div>
  </div>
);
...
export default function App() {
  ...
  return (
    <div className="min-h-screen" style={{ background: "#0f0e0c", fontFamily: "'Outfit', sans-serif", color: "#e8e0d0" }}>
      ...
      <header style={{ borderBottom: "1px solid #1a1814" }}>
        <div className="max-w-5xl mx-auto px-6 py-4 flex items-center justify-between">
          <div className="flex items-center gap-3">
            <div className="w-8 h-8 rounded flex items-center justify-center" style={{ background: "#8B7355" }}>
              <Icons.Zap />
            </div>
            <div>
        ...
        {!result && !loading && (
          <div className="max-w-2xl mx-auto">
            <div className="mb-8">
              ...
              <p className="mt-2 text-sm" style={{ color: "#5a5248" }}>Заполните бриф — Claude создаст ТЗ и концепцию</p>
            </div>
            ...
            <div className="space-y-6">
              ...
              <div className="grid grid-cols-2 gap-4">
                <div className="col-span-2 sm:col-span-1">
                  <Label>Ниша</Label>
                  <Input value={form.niche} onChange={setField("niche")} placeholder="Фитнес, курсы, косметика..." />
                </div>
                <div className="col-span-2 sm:col-span-1">
                  <Label>Язык креатива</Label>
                  <Input value={form.language} onChange={setField("language")} placeholder="Русский" />
                </div>
              </div>

              <div>
                <Label>Информация о продукте</Label>
                <Textarea value={form.productInfo} onChange={setField("productInfo")} placeholder="Что за продукт, цена, характеристики..." rows={3} />
              </div>

              <div>
                <Label>Главный оффер / суть</Label>
                <Textarea value={form.mainProductMessage} onChange={setField("mainProductMessage")} placeholder="Главная мысль для донесения..." rows={2} />
              </div>

              <div>
                <Label optional>Целевая аудитория</Label>
                <Textarea value={form.audienceInfo} onChange={setField("audienceInfo")} placeholder="Кто покупает, возраст, боли, желания..." rows={2} />
              </div>

              <div className="grid grid-cols-2 gap-4">
                <div className="col-span-2 sm:col-span-1">
                  <Label>Стиль (2–5 слов)</Label>
                  <Input value={form.style} onChange={setField("style")} placeholder="Минимализм, яркий..." />
                </div>
                <div className="col-span-2 sm:col-span-1">
                  <Label optional>Акценты</Label>
                  <Input value={form.focusPoints} onChange={setField("focusPoints")} placeholder="На что делать упор..." />
                </div>
              </div>

              <div>
                <Label optional>Пожелания</Label>
                <Textarea value={form.wishes} onChange={setField("wishes")} placeholder="Особые требования..." rows={2} />
              </div>

              {error && (
                <div className="px-4 py-3 rounded text-sm" style={{ background: "#e55a4e15", border: "1px solid #e55a4e40", color: "#e55a4e" }}>
                  {error}
                </div>
              )}

              <button onClick={() => generate()}
                className="w-full py-4 font-bold text-sm tracking-widest uppercase flex items-center justify-center gap-3 rounded transition-all"
        ...
        {result && !loading && (
          <div>
            <div className="flex items-center justify-between mb-6">
              ...
              <button onClick={() => { setResult(null); setRevisionMode(false); }}
              ...
              <div className="mb-4 p-5 rounded-lg" style={{ background: "#1a1814", border: "1px solid #2d2a25" }}>
                ...
                <div className="flex gap-3 mt-4">
                  <button onClick={handleRevision}
                    className="flex-1 py-3 font-bold text-xs tracking-widest uppercase rounded flex items-center justify-center gap-2"
            ...
            {!revisionMode && (
              <div className="flex flex-wrap gap-3">
                <button onClick={saveToHistory} className="flex items-center gap-2 px-5 py-3 rounded text-sm font-semibold"
                  style={{ background: "#8B7355", color: "#0f0e0c" }}>
                  <Icons.Check /> Готово
                </button>
                <button onClick={() => setRevisionMode(true)} className="flex items-center gap-2 px-5 py-3 rounded text-sm"
                  style={{ border: "1px solid #2d2a25", color: "#e8e0d0", background: "#1a1814" }}>
                  <Icons.Pencil /> Правки
                </button>
                <button onClick={downloadResult} className="flex items-center gap-2 px-5 py-3 rounded text-sm"
                  style={{ border: "1px solid #2d2a25", color: "#e8e0d0", background: "#1a1814" }}>
      ...
      {showHistory && <HistoryPanel history={history} onSelect={(item) => { setForm(item.form); setResult(item.result); }} onClose={() => setShowHistory(false)} />}
    </div>
  );
}
